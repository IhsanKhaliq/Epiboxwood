...
humidity, daily minimum temperatures, and daily maximum wind speed  over the
trial duration.
...
Wind speed and trial duration had no significant effect. Increasing temperature
and humidity has a significant positive linear relationship with disease
severity. Rainfall effect has a significant positive effect on disease severity,
but a decreased in disease severity is predicted after 6 mm/rainy day.
...
Based on correlation analysis, daily minimum temperature and daily maximum wind
speed during rainy days has a higher correlation with disease, so use these
variables instead of mean values for daily minimum temperatures and daily
maximum wind speed over the trial duration.
...
Use the number of days with relative humidity over 65% as a predictor, which has
the second highest correlation with the disease after after maximum relative
humidity during dry conditions.
...
Same output as `m1` and `m2`, but trial duration has a significant effect. This
means trial duration is important when number of days with relative humidity
over 65% is used as a predictor, so this predictor is not normalising the data
for varying trial period.
...
Only rain has no significant effect. Trial duration has a significant effect.
This means trial duration is important when number of days with relative
humidity over 65% is used as a predictor, so this predictor is not normalising
the data for varying trial period.
...
The `k` values are greater than `edf` and p-values are not significant. More
data will further increase the fit.
...
The plot for interaction effect between RH and temperature suggests a negative
interaction effect. This means that the effect of RH on disease_severity changes
depending on the value of temperature. When temperature is low, an increase in
RH is associated with a larger increase in disease_severity than when
temperature is high.
...
The plot for the interaction effect between max_ws and avg_daily_rain suggests a
negative interaction effect. This means that the effect of max_ws on
disease_severity changes depending on the value of avg_daily_rain. When
avg_daily_rain is low, an increase in max_ws is associated with a larger
increase in disease_severity than when avg_daily_rain is high. No increase in
disease severity is recorded, at 6 mm rain per rainy day, with increasing wind
speed.
